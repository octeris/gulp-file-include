...
module.exports = function(opts) {
  ...
  function include(file, text, data) {
    ...
    function conditionalHandler(inst) {
      try {
        var cond = replaceVariable(inst.args, data, opts);
        var condition = new Function('var context = this; with (context) { return ' + cond + '; }').call(data) // eslint-disable-line
      } catch (error) {
    ...
    function loadJsonFromFile(arg) {
          var jsonfile = file.base + arg.replace(/^('|")/, '').replace(/('|")$/, '');
    ...
    function includeHandler(inst) {
      var args = /[^)"']*["']([^"']*)["'](?:,\s*(?:["']([^"']*)["']|({[\s\S]*}))){0,1}(?:,\s*(?:["']([^"']*)["']|({[\s\S]*}))){0,1}\s*/.exec(inst.args)
      ...
      if (args) {
        ...
        if (args) {
          ...
          if(args[4]) {
						json = extend({}, json, loadJsonFromFile(args[4]));
					} else if(args[5]){
						json = extend({}, json, SON.parse(args[5]));
					}
        }

        recFile = include(recFile, includeContent, json)
